...
import net.ygbstudio.postdirector.exceptions.UnknownAPICredentials;
import net.ygbstudio.postdirector.utils.Helpers;

/**
 * RESTful web service for managing post metadata in the PostDirector
 * application.
...
public class PostDirectorDataSourceRest {
	 ...
	 * @throws UnknownAPICredentials 
	 */
	@GET
	@Path("{postID: [0-9]+}")
	@RolesAllowed(value = { "user" })
	@Produces(MediaType.APPLICATION_JSON)
	public String getPost(@PathParam("postID") Long postId) throws UnknownAPICredentials {
	 ...
	 * 
	 * @param postMetaFields | the ClientPostMeta object containing post metadata to
	 *                       update
	...
	public Response updatePost(@PathParam("postID") Long postID, ClientPostMeta postMetaFields) {
	 ...
	 * 
	 * @param clientPost | the ClientPostMeta object containing post metadata to
	 *                   update
	 */
				...
				.forEach(p -> {
					switch (PostMetaKeys.fromValue(p).get()) {
						case POSTID:
							break;
						case HOURS:
							dbDao.updatePostMetaAuto(postId, PostMetaKeys.HOURS.toString(),
									Long.toString(clientPost.getHours()));
							break;
						case MINUTES:
							dbDao.updatePostMetaAuto(postId, PostMetaKeys.MINUTES.toString(),
									Long.toString(clientPost.getMinutes()));
							break;
						case SECONDS:
							dbDao.updatePostMetaAuto(postId, PostMetaKeys.SECONDS.toString(),
						...
						case EMBED:
							...
							break;
						case PRODUCTION:
							dbDao.updatePostMetaAuto(postId, PostMetaKeys.PRODUCTION.toString(),
									clientPost.getVideoProduction());
							break;
						case ORIENTATION:
							dbDao.updatePostMetaAuto(postId, PostMetaKeys.ORIENTATION.toString(),
									clientPost.getVideoOrientation());
							break;
						case ETHNICITY:
							dbDao.updatePostMetaAuto(postId, PostMetaKeys.ETHNICITY.toString(),
									clientPost.getEthnicity());
							break;
						case HAIRCOLOR:
							dbDao.updatePostMetaAuto(postId, PostMetaKeys.HAIRCOLOR.toString(),
									clientPost.getHairColor());
							break;
						case HDVIDEO:
							dbDao.updatePostMetaAuto(postId, PostMetaKeys.HDVIDEO.toString(),
									clientPost.getVideoHD() ? "on" : "off");
							break;
						case THUMBNAIL:
							dbDao.updatePostMetaAuto(postId, PostMetaKeys.THUMBNAIL.toString(),
									clientPost.getThumbURI());
							break;
						case VIDEOURL:
							dbDao.updatePostMetaAuto(postId, PostMetaKeys.VIDEOURL.toString(),
